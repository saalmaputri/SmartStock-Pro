...
export default function Transactions() {
  ...
  const [form, setForm] = useState({ product_id: '', warehouse_id: '', supplier_id: '', quantity: 1, transaction_type: 'in', notes: '' })
  ...
  async function load() {
    setLoading(true)
    try {
      const [tx, productRes, warehouseRes, supplierRes] = await Promise.all([api.get('/transactions'), api.get('/products'), api.get('/warehouses'), api.get('/suppliers')])
      setRows(Array.isArray(tx.data) ? tx.data : [])
      ...
      setSuppliers(Array.isArray(supplierRes.data) ? supplierRes.data : supplierRes.data.items || [])
    } catch (err) {
      setRows([])
      setProducts([])
      setWarehouses([])
  ...
  async function submit(event) {
    ...
    try {
      await api.post('/transactions', {
        ...
        supplier_id: form.transaction_type === 'in' && form.supplier_id ? Number(form.supplier_id) : null,
        quantity: Number(form.quantity)
      })
      setForm({ ...form, supplier_id: '', quantity: 1, notes: '' })
      await load()
    } catch (err) {
      setError(errorMessage(err))
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={submit} className="card grid gap-4 p-6 md:grid-cols-5">
        <label className="grid gap-2"><span className="label">Tipe</span><select className="field" value={form.transaction_type} onChange={(e) => setForm({ ...form, transaction_type: e.target.value, supplier_id: e.target.value === 'out' ? '' : form.supplier_id })}><option value="in">Barang Masuk</option><option value="out">Barang Keluar</option></select></label>
        ...
        {form.transaction_type === 'in' && (
          <label className="grid gap-2"><span className="label">Supplier</span><select className="field" value={form.supplier_id} onChange={(e) => setForm({ ...form, supplier_id: e.target.value })}><option value="">Pilih Supplier</option>{suppliers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
        )}
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: 'supplier_name', label: 'Supplier', render: (row) => row.transaction_type === 'in' ? (row.supplier_name || '-') : '-' },
